 ...
 * 
 */
public class DBFormel1_HSQL {

	DB_HSQL db;

	public DBFormel1_HSQL() {
		db = new DB_HSQL();
	}
	...
	public void dbAufgabe() {
		...
		// Zugriff ueber Netz
		// if (!db.conOeffnenServer("localhost/xdb", "SA", "")) return;

		// Zugriff auf Dateisystem
		if (! db.conOeffnenDatei("f1/f1", "SA", "") ) return ;

		// Zugriff auf Dateisystem - DB muss existieren
		// if (! db.conOeffnenExistierendeDatei("f1/f1", "SA", "") ) return ;

		// Zugriff auf InMemmoryDB
		// if (! db.conOeffnenInMemmory("f1", "SA", "") ) return ;

		// db.conExecute("CREATE Database IF NOT EXISTS Formel1;"); // Nicht bei
		// HSQLDB
		// db.conExecute("USE Formel1"); // Nicht bei HSQLDB
	...
	public static void main(String[] args) {
		DBFormel1_HSQL t = new DBFormel1_HSQL();
		t.dbAufgabe();
	}
}
